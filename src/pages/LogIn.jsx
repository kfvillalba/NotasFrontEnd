...
const LogIn = () => {
  ...
  const ejecutarCreacionCategoria = (email) => {
    fetch(
      `http://localhost:5272/notes-service/Categorias/CrearSinCategoria?email=${email}`,
      {
        method: 'POST',
        headers: {
  ...
  const logIngEmailPassword = (email, password) => {
    fetch(
      `http://localhost:5272/authetication-service/Usuarios/Autenticacion?email=${email}&password=${password}`
    )
      .then((response) => {
        if (response.ok) {
      ...
      .then((data) => {
        ...
        ejecutarCreacionCategoria(email)
        fetch('http://localhost:7273/EnviarEmail', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const logIngGoogle = () => {
    signInWithPopup(auth, googleProvider)
      .then((data) => {
        ...
        ejecutarCreacionCategoria(data.user.email)
        fetch('http://localhost:7273/EnviarEmail', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          ...
          body: JSON.stringify({
            ...
            body: 'Si no fuiste tú, cambia de inmediato tu clave.',
            attachments: ['dfgdfg'],
          }),
        })
          .then((response) => {
