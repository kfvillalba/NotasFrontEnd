...
const ModalEditProveedores = ({ open, onClose, editar, dataProveedor }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const onSubmit = async (data) => {
    try {
      const response = await fetch(
        `https://localhost:7127/api/Proveerdors/Actualizar?id=${dataProveedor.id}`,
        {
          method: "PUT",
          headers: {
      ...
      <div className="">
        ...
        >
          <div>
            ...
            <input
              defaultValue={`${dataProveedor.nombre}`}
              id="nombre"
              type="text"
...
export default ModalEditProveedores;
